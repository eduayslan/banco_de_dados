...
-- INNER JOIN; RETORNA REGISTROS QUANDO HA PELO MENOS UMA CORRESPONDENCIOA EM AMBAS TABELAS --
...
select clientes.nome, pedidos.id as pedido from pedidos left join clientes on cliente_id = pedidos.cliente_id;
 
 -- 2 Quantos pedidos foram feitos no total?
 select count(*) from pedidos; 
 
 -- 3 Liste os pedidos feitos após '2016-12-15' com o nome do cliente.
 select pedidos.id as pedido, pedidos.data, clientes.nome from pedidos inner join clientes on pedidos.cliente_id = clientes.id 
 where pedidos.data > '2026-12-15 23:59:59';
...
select nome, sum(pedidos.id) from clientes inner join pedidos on clientes.id = pedidos.cliente_id group by nome;
...
select clientes.nome, pedidos.id as pedido, pedidos.data, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id = cliente_id
...
select nome, sum(valor) from clientes inner join pedidos on clientes.id = pedidos.cliente_id group by nome;
...
select nome, sum(quantidade) as total_pizzas, count(pedidos.id) as total_pedidos from itens_pedido inner join pedidos on itens_pedido.pedido_id = pedidos.id
...
/*1*/
 ...
 select clientes.nome, pedidos.id as pedido, pedidos.data, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id = cliente_id
 where pedidos.data between '2016-12-01' and '2016-12-31 23:59:59';
 /*between serve para comparar se algo é maior ou menor que o outro*/
 
 /*14*/
select nome, pedidos.id as pedido, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id = clientes.id order by valor desc;

/*19*/
select nome, pedidos.id as pedido, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id = clientes.id order by valor asc;
...
select clientes.nome, count(pedidos.id) as total_pedidos from pedidos inner join clientes on clientes.id group by clientes.id, clientes.nome;
